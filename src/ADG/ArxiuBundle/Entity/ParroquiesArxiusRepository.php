...
class ParroquiesArxiusRepository extends EntityRepository
{
	...
	public function findParroquies($nodac, $data, $nom, $paraula){
		...
		$qb->select('p.id,p.nodac,p.data,p.nom,p.titol')->from('ArxiuBundle:ParroquiesArxius', 'p');
	...
	public function findDetalls($nodac){
		...
		$qb->select('p.nodac,p.data,p.nom,p.titol,p.unitat,p.volum,p.mides,p.cobertes,p.pagines,p.ambIndex,p.acces,p.notes,
				p.estat,p.llengua,p.autors,p.fitxa,p.dataIngres')->from('ArxiuBundle:ParroquiesArxius', 'p');
